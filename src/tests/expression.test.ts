...
describe("ExpressionEvaluator", () => {
	...
	describe("Number Formats and Symbols", () => {
		...
		it("should resolve the program counter symbol (*)", () => {
			...
			const result = evaluator.evaluateAsNumber(tokens, { PC: { value: 0x1000 } });
	...
	describe("Bitwise Operations", () => {
		it("should evaluate bitwise AND, OR, and XOR", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize("$F0 & $88"), { PC: { value: 0 } })).toBe(0x80);
			expect(evaluator.evaluateAsNumber(tokenize("$F0 | $09"), { PC: { value: 0 } })).toBe(0xf9);
			expect(evaluator.evaluateAsNumber(tokenize("$AA ^ $FF"), { PC: { value: 0 } })).toBe(0x55);
		});

		it("should works with characters too", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize('"A" & $4f'), { PC: { value: 0 } })).toBe(0x41);
			expect(evaluator.evaluateAsNumber(tokenize('"A" | $80'), { PC: { value: 0 } })).toBe(0xc1);
		});
	...
	describe("Comparison Operators", () => {
		it("should handle numeric equality (== and =)", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize("10 == 10"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("10 = 10"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("10 == 5"), { PC: { value: 0 } })).toBe(0);
		});

		it("should handle numeric inequality (!=)", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize("10 != 5"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("10 != 10"), { PC: { value: 0 } })).toBe(0);
		});

		it("should handle numeric relational operators (<, >, <=, >=)", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize("20 > 10"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("10 < 20"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("20 < 10"), { PC: { value: 0 } })).toBe(0);
			expect(evaluator.evaluateAsNumber(tokenize("10 > 20"), { PC: { value: 0 } })).toBe(0);
			expect(evaluator.evaluateAsNumber(tokenize("10 <= 10"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("10 <= 20"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("20 <= 10"), { PC: { value: 0 } })).toBe(0);
			expect(evaluator.evaluateAsNumber(tokenize("10 >= 10"), { PC: { value: 0 } })).toBe(1);
		...
		it("should handle string inequality (!=)", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize('"hello" != "world"'), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize('"hello" != "hello"'), { PC: { value: 0 } })).toBe(0);
		});
	});
	...
	describe("Boolean Logic Operators", () => {
		it("should handle logical NOT (!)", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize("!10"), { PC: { value: 0 } })).toBe(0);
			expect(evaluator.evaluateAsNumber(tokenize("!0"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("!(10 > 5)"), { PC: { value: 0 } })).toBe(0); // !(true) -> 0
			expect(evaluator.evaluateAsNumber(tokenize("!(5 > 10)"), { PC: { value: 0 } })).toBe(1); // !(false) -> 1
		});

		it("should handle logical AND (&&)", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize("1 && 1"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("10 && 0"), { PC: { value: 0 } })).toBe(0);
			expect(evaluator.evaluateAsNumber(tokenize("(5 > 3) && (10 > 5)"), { PC: { value: 0 } })).toBe(1);
		});

		it("should handle logical OR (||)", () => {
			const { evaluator, tokenize } = setup();
			expect(evaluator.evaluateAsNumber(tokenize("1 || 0"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("0 || 0"), { PC: { value: 0 } })).toBe(0);
			expect(evaluator.evaluateAsNumber(tokenize("10 || -1"), { PC: { value: 0 } })).toBe(1);
			expect(evaluator.evaluateAsNumber(tokenize("(5 < 3) || (10 > 5)"), { PC: { value: 0 } })).toBe(1);
			expect(
	...
	describe("Array Indexing", () => {
		...
		it("should throw error when indexing a non-array", () => {
			...
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Attempted to index a non-array value on line 1.");
		});

		it("should throw error for out-of-bounds index", () => {
			const { evaluator, tokenize } = setup();
			const tokens = tokenize("[1,2,3][3]");
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Array index 3 out of bounds for array of length 3 on line 1.");
		});

		it("should throw error for non-numeric index", () => {
			const { evaluator, tokenize } = setup();
			const tokens = tokenize('[1,2,3]["hello"]');
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Array index must be a number on line 1.");
		});
	});
	...
	describe("Error Handling", () => {
		it("should throw on division by zero", () => {
			...
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Division by zero.");
		});

		it("should throw on undefined symbol when forward refs are not allowed", () => {
			const { evaluator, tokenize } = setup();
			const tokens = tokenize("UndefinedSymbol");
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Undefined symbol 'UNDEFINEDSYMBOL' on line 1.");
		});
		...
		it("should throw on mismatched parentheses", () => {
			...
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Mismatched parenthesis: unmatched '(' left in stack.");
		});

		it("should throw when a number is expected but a string is found", () => {
			const { evaluator, tokenize } = setup();
			const tokens = tokenize('"this is not a number"');
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Expression did not evaluate to a number as expected.");
		});

		it("should throw on invalid expression format", () => {
			const { evaluator, tokenize } = setup();
			const tokens = tokenize("5 5 +");
			expect(() => evaluator.evaluateAsNumber(tokens, { PC: { value: 0 } })).toThrow("Invalid expression format: Unexpected token '5' on line 1.");
		});
	});
