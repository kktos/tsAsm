...
export class LoopDirective implements IDirective {
	...
	private handleForLoop(directive: ScalarToken, context: DirectiveContext): void {
		...
		const itemIteratorToken = this.assembler.parser.identifier();
		const ofToken = this.assembler.parser.identifier("OF");
		...
		const exprHeader = this.assembler.parser.getInstructionTokens();
		...
		this.assembler.symbolTable.pushScope(`@@for_${directive.line}_`);
	...
	private handleRepeatLoop(directive: ScalarToken, context: DirectiveContext): void {
		const countExpressionTokens = this.assembler.parser.getInstructionTokens();
		...
		this.assembler.symbolTable.pushScope();
	...
	private runNextLoopIteration(loopId: string, context: DirectiveContext): void {
		...
		if (state.repeatCount !== undefined && state.repeatCount > 0) {
			...
			if (state.iterator) {
				...
				this.assembler.symbolTable.assignVariable(state.iterator.value, iteratorValue);
			}

			// Handle .FOR specific item value
			if (state.items && state.itemIterator) {
				const currentItem = state.items[currentIndex] as SymbolValue;
				this.assembler.symbolTable.assignVariable(state.itemIterator.value, currentItem);
			}
